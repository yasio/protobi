    ...
    namespace aes {
        ...
        namespace detail {
            ...
            namespace padding{
                ...
                {
                    static_assert(sizeof(typename _BYTE_SEQ_CONT::value_type) == 1, "PKCS7: only allow allow stl sequently byte conatiner!");
                ...
                {
                    static_assert(sizeof(_BYTE_SEQ_CONT::value_type) == 1, "ZEROS: only allow allow stl sequently byte conatiner!");
                    size_t padding_size = blocksize - plaintext.size() % blocksize;
                    for (size_t offst = 0; offst < padding_size; ++offst)
                    {
                ...
                {
                    static_assert(sizeof(_BYTE_SEQ_CONT::value_type) == 1, "ANSIX923: only allow allow stl sequently byte conatiner!");
                    size_t padding_size = blocksize - plaintext.size() % blocksize;
                ...
                {
                    static_assert(sizeof(_BYTE_SEQ_CONT::value_type) == 1, "ISO10126: only allow allow stl sequently byte conatiner!");
                    size_t padding_size = blocksize - plaintext.size() % blocksize;
                    for (size_t offst = 0; offst < padding_size - 1; ++offst)
                    {
            ...
            /// for partial encrypt/decrypt
            void set_encrypt_key(const char* private_key, int keybits = 256);

            void set_decrypt_key(const char* private_key, int keybits = 256);

            void set_ivec(void);

            void cbc_encrypt_partial(const char* in, size_t inlen,
                char* out, size_t outlen);

            void cbc_decrypt_partial(const char* in, size_t inlen,
                char* out, size_t outlen);
