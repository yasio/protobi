...
public:
    ...
    void _Generate_constants_lua()
    {
        ...
        // _Write_start(this->constants_decl, (prefix + "constants").c_str());
        this->constants_lua << "pcode_autog = pcode_autog or {}\n\n";
        for (auto iter = this->pi.messages.begin(); iter != this->pi.messages.end(); ++iter)
        {
            if (!iter->is_header && !iter->command_id.empty())
                this->constants_lua << "pcode_autog." << (*iter).command_id << ";\n";
    ...
    void _Generate_messages_impl(void)
    {
        ...
        for (auto mi = this->pi.messages.begin(); mi != this->pi.messages.end(); ++mi)
        {
            ...
            if (mi->mode & MODE_ENCODE)
            { // encode
                this->messages_impl << "obinarystream " << mi->name << "::encode(void) const\n{\n";
                // this->messages_impl << "    " << "obinarystream obs;\n";
                ...
                if (!mi->is_header)
                {
                    ...
                    this->messages_impl << ident(1) << "obinarystream obs;\n\n";
                }
                ...
                this->messages_impl << ident(1) << "return std::move(obs);\n";
            ...
            { // decode
                ...
                this->messages_impl << "    " << "ibinarystream ibs;\n";
                ...
                if (mi->is_header || !mi->zlib)
                {
                    ...
                    this->messages_impl << "    " << "ibs.vassign(uncompr.data(), uncompr.size());\n\n";
                }
...
int main(int argc, char** argv)
{
    ...
    header.command_id = 101;
    header.version = 1;
    header.reserved = 0x6102;
    header.reserved2 = time(nullptr);

    messages::LoginReq req;
    req.user_flag = false;
    req.user_id = 201;
    strcpy(req.device_id, "hello world!");
    ...
    messages::HwpHeader check_hdr;
    messages::LoginReq check_req;

    auto remain = check_hdr.decode(data.data(), data.size());
    remain = check_req.decode(&data.back() - remain + 1, remain);
#endif
