...
public:
    ...
    simple_ptr(const _Myt& _Right) = delete;
    simple_ptr(_Myt&& _Right) throw()
        : _Myptr(_Right.release())
    {    // construct by assuming pointer from _Right simple_ptr
    }

    _Myt& operator=(const _Myt& _Right) = delete;
    ...
    simple_ptr(const simple_ptr<_T, _Dr>& _Right) = delete;
    template<typename _T, typename _Free<_Ty>::type _Dr>
    simple_ptr(simple_ptr<_T, _Dr>&& _Right) throw()
    ...
    template<typename _T, typename _Free<_Ty>::type _Dr>
    _Myt& operator=(const simple_ptr<_T, _Dr>& _Right) = delete;

    template<typename _T, typename _Free<_Ty>::type _Dr>
    _Myt& operator=(simple_ptr<_T, _Dr>&& _Right) throw()
    {    // assign compatible _Right (assume pointer)
#ifdef __cxx11
        static_assert(std::is_same<_Myt, decltype(_Right)>::value, "error, the deleting behavior must be consistent!");
#endif
