...
static const socket_handle bad_sock = (socket_handle)-1;
...
public:
    ...
    */
    ...
    socket_handle get_handle(void) const;

    socket_handle native_handle(void) const;

        // release this;
    socket_handle release_handle(void);
    ...
    */
    int set_nonblocking(void) const;
    ...
    */
    int send_n(const void* buf, int len, timeval* timeout, int flags = 0, int max_retry_times = 3);
    static int send_n(socket_handle s, const void* buf, int len, timeval* timeout, int flags = 0, int max_retry_times = 3);
    ...
    */
    ...
    static int send_i(socket_handle fd, const void* buf, int len, int flags = 0);
    ...
    */
    ...
    static int handle_write_ready(socket_handle s, timeval* timeo);
    ...
    */ 
    ...
    static ip::endpoint_v4 get_local_addr(socket_handle);
    ...
    */
    ...
    static ip::endpoint_v4 get_peer_addr(socket_handle);
    ...
    */
    ...
    static int set_keepalive(socket_handle s, int flag = 1, int idle = 7200, int interval = 75, int probes = 10);
    ...
    */
    ...
    template<typename T>
    static int set_optval(socket_handle, int level, int optname, const T& optval);
    ...
    */
    ...
    template<typename T>
    static int get_optval(socket_handle s, int level, int optname, T& optval, socklen_t = sizeof(T));
    ...
    */
    ...
    operator socket_handle(void) const;
...
template<typename T> inline
    int xxsocket::set_optval(socket_handle s, int level, int optname, const T& optval)
{
    return ::setsockopt(s, level, optname, (char*)&optval, sizeof(T));
}

template<typename T> inline
    int xxsocket::get_optval(socket_handle s, int level, int optname, T& optval, socklen_t optlen)
{
...
/// automic defs
#ifdef _MSC_VER
#define automic_inc32(vval) InterlockedIncrement(&vval)
#define automic_dec32(vval) InterlockedDecrement(&vval)
#endif
#ifdef __GNUC__
#define automic_inc32(vval)    \
    __asm__ __volatile__(      \
        "lock incl %0"         \
        : "+m"(vval)           \
        )
#define automic_dec32(vval)    \
    __asm__ __volatile__(      \
        "lock decl %0"         \
        : "+m"(vval)           \
        )
...
}; /* namespace: simplepp_1_13_201301::net */
